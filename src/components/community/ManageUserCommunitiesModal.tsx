...
export default function ManageUserCommunitiesModal({
  ...
  const handleRemoveFromCommunity = async (commId: string) => {
    ...
    setConfirmModal({
      ...
      message: isAr ? `هل أنت متأكد من إزالة ${user.fullName} من هذا المجتمع؟` : `Are you sure you want to remove ${user.fullName} from this community?`,
      onConfirm: async () => {
  ...
  const handleMoveCommunity = async (fromCommId: string) => {
    ...
    setConfirmModal({
      isOpen: true,
      title: isAr ? "تأكيد النقل" : "Confirm Move",
      message: isAr ? `هل أنت متأكد من نقل ${user.fullName} إلى المجتمع الجديد؟` : `Are you sure you want to move ${user.fullName} to the new community?`,
      onConfirm: async () => {
                          ...
                          {availableCommunities.length > 0 && (
                            <div className="pt-2 border-t border-slate-100 dark:border-slate-700/60 flex items-center gap-2 flex-wrap sm:flex-nowrap">
                              <CustomSelect
                                ...
                                onChange={(val) => setMoveTargets(prev => ({ ...prev, [commId]: val }))}
                                placeholder={isAr ? "اختر مجتمع للنقل إليه..." : "Select community to move to..."}
                                options={availableCommunities.map(ac => ({ value: ac.id, label: ac.name }))}
                                className="flex-1"
                              />
                              <button
                ...
                <div className="bg-slate-50 dark:bg-slate-800/50 rounded-2xl p-4 border border-slate-200 dark:border-slate-700">
                  ...
                  <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
                    <CustomSelect
                      ...
                      onChange={(val) => setSelectedAddComm(val)}
                      placeholder={isAr ? "اختر مجتمع للإضافة..." : "Select community to add..."}
                      options={availableCommunities.map(ac => ({ value: ac.id, label: ac.name }))}
                      className="flex-1"
                    />
                    <button
