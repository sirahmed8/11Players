              ...
              <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm">
                ...
                <form onSubmit={handleCreateCommunity} className="space-y-4">
                  <div>
                    ...
                    <input required value={newCommName} onChange={e => setNewCommName(e.target.value)} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-600" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-1">Description</label>
                    <input required value={newCommDesc} onChange={e => setNewCommDesc(e.target.value)} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-600" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold mb-1">Admin UID (User ID)</label>
                    <input required value={newCommAdmin} onChange={e => setNewCommAdmin(e.target.value)} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-600" />
                  </div>
                  <label className="flex items-center gap-2">
                    <input type="checkbox" checked={isPrivate} onChange={e => setIsPrivate(e.target.checked)} />
                    <span className="text-sm font-semibold">Is Private</span>
                  </label>
                  {isPrivate && (
                    <div>
                      <label className="block text-sm font-semibold mb-1">Password</label>
                      <input required value={password} onChange={e => setPassword(e.target.value)} className="w-full px-3 py-2 bg-slate-100 dark:bg-slate-900 rounded border border-slate-300 dark:border-slate-600" />
                    </div>
                  )}
