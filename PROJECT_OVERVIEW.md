...
## 13. Milestone M2.6 — Domain Directive, Mic Permission, Leaderboard Merge & Profile Features Pass
- **11AI Domain Directive & Academic Banning (`src/app/api/ai/chat/route.ts`)**:
  - Enforced a strict Football/Soccer Tactical Analyst domain directive in 11AI system prompt. Explicitly banned all academic terms, study habits, semester progress, exams, courses, and "Obour Academic Hub" references from AI replies and follow-up `[SUGGESTIONS]`.
- **Browser Microphone Permission Trigger (`src/components/ui/FloatingChatWidget.tsx`)**:
  - Upgraded `toggleMicListening` to invoke `await navigator.mediaDevices.getUserMedia({ audio: true })` prior to starting speech recognition, forcing the native browser permission prompt (Allow/Decline microphone access) to pop up instantly.
- **Solid Color Leaderboard Header & Podium Cards (`src/app/stats/page.tsx`)**:
  - Eliminated multi-color text and background gradients (`from-`, `to-`, `bg-gradient-to-r`) on Leaderboard headers, podium cards, and badges in favor of solid, sleek, premium colors (`bg-slate-900`, `bg-amber-500`, `bg-emerald-600`, `text-white`, `border-slate-800`).
- **Merged Season Ceremony into Leaderboard Page (`src/app/stats/page.tsx` & `src/app/season-ceremony/page.tsx`)**:
  - Merged `SeasonCeremony` directly into `/stats` and `/leaderboard` with a primary tab switcher (`🏆 Leaderboard & Rankings` vs `👑 Season Ceremony & Archives`). Re-exported `/season-ceremony` to redirect to `/leaderboard?tab=ceremony`.
- **Automated Profile Completion Guard (`src/components/auth/ProtectedRoute.tsx`)**:
  - Added an automated profile completion guard in `ProtectedRoute.tsx`. Users with incomplete profile registration details (`!cardName`, `!position`, or `onboardingCompleted !== true`) are automatically redirected to `/onboarding` upon page load or refresh until their profile is complete.
- **Online Presence Indicator Fix (`src/components/ui/FloatingChatWidget.tsx`)**:
  - Ensured online presence dots render bright emerald green (`bg-emerald-500 shadow-emerald-500/50`) when users are active online.
- **All Features Quick Hub on Profile Page (`src/app/profile/page.tsx`)**:
  - Added a dedicated "All Career Features & Tools" quick hub bar on `/profile` linking to Skill Tree, Achievements, Kit Builder, Leaderboard & Ceremony, Derby H2H, Split Bill, and Account Settings.
